        ...
        if current_winrate is not None:
            ...
            if all(v == 0 for v in self.cooldown.values()):
                ...
                if (current_winrate >= self.winrate_threshold) & (
                        current_lvl_idx < len(self.curriculum[sample_matchup]["levels"]) - 1):
                    # export model
                    model_path = os.path.join(self.save_path, f"final_model_lvl{current_difficulty}_winrate_{current_winrate}.zip")
